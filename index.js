...
const calculateForYear = (year) => {
    ...
    for (let month = 0; month < 5; month++) {
        ...
        // console.log(iStr)
        yearlyCalc.push(
            calculateMarksPromise(iStr, month)
            )
    }
    return Promise.all(yearlyCalc).then((data) => {
        ...
        const cagr = data.reduce((retcagr, totalRet) => {
            ...
            const periodCagrLarge = retcagr * ((100 + totalRet.totalReturnsLarge) / 100)
            ...
            const periodCagrSmall = retcagr * ((100 + totalRet.totalReturnsSmall) / 100)
